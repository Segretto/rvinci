...
def load_bottleneck_frame(run_dir: Path, output_dir_name: str):
    ...
    if not filename.exists():
        raise FileNotFoundError(f"File {filename} not found. Please record fixed frame first with 'f'")
...
def generate_random_position(bottleneck_frame: SE3Pose, traj_config: SpotTrajectoryConfig):
    ...
    roll = np.random.uniform(-math.radians(traj_config.roll_range_deg), math.radians(traj_config.roll_range_deg))

    # Calculate translation in BODY frame (additive)
    x = bottleneck_frame.x - x_distance
    ...
    return SE3Pose(x=x, y=y, z=z, rot=final_quat)

def generate_plane_position(bottleneck_frame: SE3Pose, traj_config: SpotTrajectoryConfig):
    """Generates a position on the bottleneck plane (fixed x, reduced y/z variations, random rotation)."""
    y_offset = np.random.triangular(-traj_config.y_range_plane, 0, traj_config.y_range_plane)
    z_offset = np.random.triangular(-traj_config.z_range_plane, 0, traj_config.z_range_plane)
    roll_range_rad = math.radians(traj_config.roll_range_deg)
    roll = np.random.triangular(-roll_range_rad, 0, roll_range_rad)

    # Keep x fixed from bottleneck, vary y and z in BODY frame
    x = bottleneck_frame.x
    y = bottleneck_frame.y + y_offset
    z = bottleneck_frame.z + z_offset
...
class JointAndFrameRecorder:
    ...
    def _gather_randomized_trajectories(self, spot_config: SpotClientConfig, traj_config: SpotTrajectoryConfig):
        log.info("Loading bottleneck frame...")
        try:
            bottleneck_frame, recorded_body_height = load_bottleneck_frame(self.run_dir, spot_config.output_dir)
            log.info(f"Bottleneck frame loaded (recorded_body_h={recorded_body_height:.3f})")
        except Exception as e:
        ...
        log.info(f"Standing up robot to recorded height: {recorded_body_height:.3f} m...")
        stand_cmd = RobotCommandBuilder.synchro_stand_command(body_height=recorded_body_height)
        ...
        gripper_cmd = RobotCommandBuilder.claw_gripper_open_fraction_command(open_fraction=1.0)
        ...
        log.info(f"Starting dataset collection with {traj_config.num_positions} positions at {frames_hz} Hz...")
        ...
        for idx in range(traj_config.num_positions):
            ...
            log.info(f"Going to position {idx + 1}/{traj_config.num_positions}: {target_pose}")
            
            robot_cmd = RobotCommandBuilder.arm_pose_command(
                x=target_pose.x, y=target_pose.y, z=target_pose.z,
                qw=target_pose.rot.w, qx=target_pose.rot.x, qy=target_pose.rot.y, qz=target_pose.rot.z,
                frame_name=BODY_FRAME_NAME,
                seconds=traj_config.move_duration,
            )
            cmd_id = self.command_client.robot_command(robot_cmd, end_time_secs=time.time() + traj_config.move_duration)
            ...
            while time.time() - start_time < traj_config.move_duration:
                ...
                robot_state = self.robot_state_client.get_robot_state()
                wrist_frame = get_a_tform_b(robot_state.kinematic_state.transforms_snapshot, BODY_FRAME_NAME, HAND_FRAME_NAME)
                hand_tform_bottleneck = calculate_gripper_to_bottleneck_transform(self.robot_state_client, bottleneck_frame)
                ...
                transform = transform_snapshot_to_dict(
                    ...
                    wrist_frame, 1.0, rel_img_filename, hand_tform_bottleneck,
                )
                ...
                if int((time.time() - start_time) * frames_hz) % frames_hz == 0:
                    distance = math.sqrt(hand_tform_bottleneck.x**2 + hand_tform_bottleneck.y**2 + hand_tform_bottleneck.z**2)
                    log.info(f"Pos {idx + 1}/{traj_config.num_positions} - Frame {len(all_rgb)}. Dist: {distance:.3f} m")
            ...
            robot_cmd = RobotCommandBuilder.arm_pose_command(
                x=plane_pose.x, y=plane_pose.y, z=plane_pose.z,
                qw=plane_pose.rot.w, qx=plane_pose.rot.x, qy=plane_pose.rot.y, qz=plane_pose.rot.z,
                frame_name=BODY_FRAME_NAME,
                seconds=traj_config.move_duration,
            )
            cmd_id = self.command_client.robot_command(robot_cmd, end_time_secs=time.time() + traj_config.move_duration)
            ...
            while time.time() - start_time < traj_config.move_duration:
                ...
                rel_img_filename = f"position_{idx + 1}_to_plane/image_{len(all_rgb)}.jpg"
                transform = transform_snapshot_to_dict(
                    image_responses[0].shot.transforms_snapshot,
                    wrist_frame, 1.0, rel_img_filename, hand_tform_bottleneck,
                )
                ...
                if int((time.time() - start_time) * frames_hz) % frames_hz == 0:
                    distance = math.sqrt(hand_tform_bottleneck.x**2 + hand_tform_bottleneck.y**2 + hand_tform_bottleneck.z**2)
                    log.info(f"Plane pos {idx + 1}/{traj_config.num_positions} - Frame {len(all_rgb)}. Dist: {distance:.3f} m")
    ...
    def run(self, spot_config: SpotClientConfig, traj_config: SpotTrajectoryConfig):
        ...
        assert self.robot.has_arm(), "Robot requires an arm to run trajectory generation"
        ...
        try:
            ...
            log.info(f"Successfully took lease for resource: {lease.resource}")
        ...
        record_thread = Thread(target=self.record_loop, args=(spot_config.frequency_hz,))
        record_thread.start()
        ...
        try:
            while True:
                ...
                if command == "q":
                    ...
                    pose = get_a_tform_b(state.kinematic_state.transforms_snapshot, BODY_FRAME_NAME, HAND_FRAME_NAME)
                    vision_tform_body = get_a_tform_b(state.kinematic_state.transforms_snapshot, VISION_FRAME_NAME, BODY_FRAME_NAME)
                    ...
                    with open(fixed_file, "w") as f:
                        json.dump({
                            "gripper_frame": {
                                ...
                                "rotation": {"w": pose.rot.w, "x": pose.rot.x, "y": pose.rot.y, "z": pose.rot.z},
                            },
                            "recorded_body_height": vision_tform_body.z,
                        }, f, indent=4)
                    log.info(f"Fixed bottleneck frame recorded (z={pose.z:.3f}, body_h={vision_tform_body.z:.3f}).")
                elif command == "g":
