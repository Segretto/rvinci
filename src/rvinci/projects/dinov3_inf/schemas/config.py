...
from rvinci.skills.perception.dinov3.schemas.config import UnifiedDINOv3Config
...
class ProjectConfig(BaseModel):
    ...
    input_source: str = Field(
        description="Camera ID (int) or Video Path/Image Path (str)"
    )
    is_video: bool = Field(
        default=False, description="Whether the input source is a video/camera stream"
    )
    ...
    dino_model_type: str = Field(
        default="dinov3_vits16plus", description="Type of DINOv3 backbone"
    )
