...
This project provides an orchestration layer to run inference using the `dinov3` skill. It supports both single image processing and real-time video/camera stream inference.
...
### 1. Image Inference
```bash
uv run python -m rvinci.projects.dinov3_inf.cli \
...
The project uses Hydra for configuration. You can find the default values in `src/rvinci/projects/dinov3_inf/configs/config.yaml`.
