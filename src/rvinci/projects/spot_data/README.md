...
```bash
uv run python -m rvinci.projects.spot_data.cli \
...
```bash
uv run python -m rvinci.projects.spot_data.cli trajectory.num_positions=50
```

Refer to `src/rvinci/projects/spot_data/schemas/config.py` for all available configuration fields.
...
All data is saved within the run directory (default: `runs/spot_data/`).
