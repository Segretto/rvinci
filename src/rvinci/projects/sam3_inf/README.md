...
```bash
uv run python -m rvinci.projects.sam3_inf.cli mode=<MODE> [mode_specific_options]
```
...
**Example:**
```bash
uv run python -m rvinci.projects.sam3_inf.cli mode=visualize \
...
**Example:**
```bash
uv run python -m rvinci.projects.sam3_inf.cli mode=tracking \
...
**Example:**
```bash
uv run python -m rvinci.projects.sam3_inf.cli mode=text_prompt \
...
**Example:**
```bash
uv run python -m rvinci.projects.sam3_inf.cli mode=filter \
...
- **Outputs**: All runs generate a unique run directory under `runs/sam3_inf/` containing logs and resolved configs.
