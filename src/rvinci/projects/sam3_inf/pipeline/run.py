...
from rvinci.projects.sam3_inf.schemas.config import ProjectConfig
from rvinci.projects.sam3_inf.pipeline.utils import (
    resolve_run_dir,
    load_coco_data,
    save_coco_format,
)
from rvinci.projects.sam3_inf.pipeline.interactive import (
    InteractiveLabeler,
    format_inputs_for_sam3,
)

from rvinci.skills.perception.sam3.api import Sam3VideoTracker, Sam3ImagePredictor
from rvinci.skills.perception.sam3.schemas.config import (
    Sam3TrackerConfig,
    Sam3PredictorConfig,
)
...
def run_tracking(config: ProjectConfig, run_dir: str):
    ...
    new_annotations = []
    next_ann_id = (
        max([ann["id"] for ann in coco_data.get("annotations", [])], default=0) + 1
    )
    ...
    for session_idx, objects in video_segments.items():
        ...
        for obj_id, mask in objects.items():
            ...
            contours, _ = cv2.findContours(
                mask_binary.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
...
def run_text_prompt(config: ProjectConfig, run_dir: str):
    ...
    for idx, prompt in enumerate(cfg.prompts):
        ...
        coco_output["categories"].append(
            {"id": cat_id, "name": prompt, "supercategory": "object"}
        )
...
def run_visualize(config: ProjectConfig, run_dir: str):
    ...
    for cat in coco.get("categories", []):
        class_map[cat["id"]] = {"name": cat["name"], "color": random_color(cat["id"])}
...
def run_pipeline(config: ProjectConfig) -> None:
    ...
    log.info(f"Starting sam3_inf in mode '{config.mode}' ... [Run Dir: {run_dir}]")
